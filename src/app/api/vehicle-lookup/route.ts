import { NextRequest, NextResponse } from "next/server";

// Israeli Ministry of Transport open data API
const GOV_API_URL = "https://data.gov.il/api/3/action/datastore_search";
const RESOURCE_ID = "053cea08-09bc-40ec-8f7a-156f0677aff3";

// Map common English/Hebrew manufacturer names to inventory names
const MAKE_MAP = new Map<string, string>([
  ["TOYOTA", "טויוטה"], ["טויוטה", "טויוטה"],
  ["HYUNDAI", "יונדאי"], ["הונדאי", "יונדאי"], ["יונדאי", "יונדאי"], ["יונדה", "יונדאי"],
  ["KIA", "קיה"], ["קיה", "קיה"],
  ["MAZDA", "מאזדה"], ["מזדה", "מאזדה"], ["מאזדה", "מאזדה"],
  ["HONDA", "הונדה"], ["הונדה", "הונדה"],
  ["NISSAN", "ניסן"], ["ניסאן", "ניסן"], ["ניסן", "ניסן"],
  ["SUZUKI", "סוזוקי"], ["סוזוקי", "סוזוקי"],
  ["MITSUBISHI", "מיצובישי"], ["מיצובישי", "מיצובישי"],
  ["VOLKSWAGEN", "פולקסווגן"], ["פולקסוואגן", "פולקסווגן"], ["פולקסווגן", "פולקסווגן"],
  ["SKODA", "סקודה"], ["SKODA AUTO", "סקודה"], ["סקודה", "סקודה"], ["שקודה", "סקודה"],
  ["SEAT", "סיאט"],
  ["MERCEDES", "מרצדס"], ["MERCEDES BENZ", "מרצדס"], ["MERCEDES-BENZ", "מרצדס"], ["מרצדס", "מרצדס"],
  ["BMW", "במו"], ["B.M.W", "במו"], ["B.M.W.", "במו"],
  ["AUDI", "אודי"], ["אודי", "אודי"],
  ["VOLVO", "וולבו"], ["וולוו", "וולבו"], ["וולבו", "וולבו"],
  ["PEUGEOT", "פיגו"], ["פיג'ו", "פיגו"], ["פיגו", "פיגו"], ["פיז'ו", "פיגו"],
  ["CITROEN", "סיטרואן"], ["CITROEN/PEUGEOT", "סיטרואן"], ["סיטרואן", "סיטרואן"],
  ["RENAULT", "רנו"], ["רנו", "רנו"],
  ["FIAT", "פיאט"], ["פיאט", "פיאט"],
  ["FORD", "פורד"],
  ["OPEL", "אופל"], ["אופל", "אופל"],
  ["ALFA ROMEO", "אלפא"], ["אלפא רומיאו", "אלפא"],
  ["JAGUAR", "יגואר"], ["יגואר", "יגואר"], ["ג'גואר", "יגואר"],
  ["LAND ROVER", "לנד רובר"],
  ["RANGE ROVER", "ריינג רובר"],
  ["LEXUS", "לקסוס"], ["לקסוס", "לקסוס"],
  ["SUBARU", "סובארו"], ["סובארו", "סובארו"],
  ["PORSCHE", "פורשה"], ["פורשה", "פורשה"],
  ["TESLA", "טסלה"], ["טסלה", "טסלה"],
  ["MINI", "מיני"], ["מיני", "מיני"],
  ["JEEP", "גיפ"], ["ג'יפ", "גיפ"], ["גיפ", "גיפ"],
  ["DACIA", "דאציה"], ["דאציה", "דאציה"], ["דצ'יה", "דאציה"],
  ["SMART", "סמארט"],
  ["ISUZU", "איסוזו"],
  ["MG", "mg"],
  ["CHEVROLET", "שברולט"],
  ["DODGE", "דודג'"],
  ["CHRYSLER", "קרייזלר"],
  ["FERRARI", "פרארי"],
  ["MASERATI", "מזארטי"],
  ["POLESTAR", "פולסטאר"],
  ["GEELY", "גילי"],
  ["CHERY", "צ'רי"], ["צרי", "צ'רי"], ["צ'רי", "צ'רי"],
  ["SSANGYONG", "סאן יאנג"],
  ["CHANGAN", "צ'אנגאן"],
  ["BYD", "byd"],
  ["GREAT WALL", "גרייט וול"],
  ["GAC", "גאקו"],
  ["CUPRA", "קופרה"],
  ["ALPINE", "אלפין"],
  ["DAIHATSU", "דייהאטסו"],
  ["CADILLAC", "קאדילק"],
  ["XPENG", "אקספנג"],
]);

// Map raw make names that include origin country
function normalizeMake(rawMake: string): string {
  // Remove country suffixes like "טסלה סין", "יונדאי קוריאה", "מזדה יפן"
  const cleaned = rawMake
    .replace(/\s*(סין|קוריאה|יפן|גרמניה|צרפת|איטליה|ספרד|שוודיה|אנגליה|צ\'כיה|רומניה|הודו|ארה"ב|ארהב|USA|CHINA|JAPAN|KOREA|GERMANY|FRANCE|ITALY|SPAIN|SWEDEN|UK|CZECH|ROMANIA|INDIA)\s*$/i, "")
    .trim()
    .toUpperCase();
  return MAKE_MAP.get(cleaned) || MAKE_MAP.get(rawMake.trim().toUpperCase()) || cleaned;
}

// Map English model names to Hebrew equivalents used in our inventory
const MODEL_MAP: Record<string, string> = {
  "MODEL 3": "מודל 3",
  "MODEL Y": "מודל Y",
  "MODEL S": "מודל S",
  "MODEL X": "מודל X",
  "COROLLA": "קורולה",
  "YARIS": "יאריס",
  "YARIS CROSS": "יאריס קרוס",
  "C-HR": "CHR",
  "CHR": "CHR",
  "RAV4": "ראב 4",
  "RAV 4": "ראב 4",
  "LAND CRUISER": "לנד קרוזר",
  "CAMRY": "קאמרי",
  "AURIS": "אוריס",
  "AYGO": "אייגו",
  "AYGO X": "אייגו X",
  "HILUX": "היילקס",
  "PRIUS": "פריוס",
  "I10": "I10",
  "I20": "I20",
  "I25": "I25",
  "I30": "I30",
  "TUCSON": "טוסון",
  "KONA": "קונה",
  "IONIQ": "איוניק",
  "IONIQ 5": "איוניק 5",
  "SANTA FE": "סנטה פה",
  "SONATA": "סונטה",
  "PICANTO": "פיקנטו",
  "RIO": "ריו",
  "SPORTAGE": "ספורטאז",
  "NIRO": "נירו",
  "STONIC": "סטוניק",
  "CEED": "סיד",
  "SORENTO": "סורנטו",
  "CARNIVAL": "קארניבל",
  "OPTIMA": "אופטימה",
  "XCEED": "אקסיד",
  "SELTOS": "סלטוס",
  "QASHQAI": "קשקאי",
  "JUKE": "גוק",
  "MICRA": "מיקרה",
  "X-TRAIL": "אקסטרייל",
  "LEAF": "ליף",
  "NAVARA": "נאברה",
  "NOTE": "נוט",
  "GOLF": "גולף",
  "POLO": "פולו",
  "TIGUAN": "טיגואן",
  "TOUAREG": "טוארג",
  "PASSAT": "פאסאט",
  "CADDY": "קאדי",
  "T-CROSS": "T CROSS טיקרוס",
  "ID.3": "ID3",
  "OCTAVIA": "אוקטביה",
  "FABIA": "פאביה",
  "SUPERB": "סופרב",
  "KAROQ": "קארוק",
  "KAMIQ": "קאמיק",
  "KODIAQ": "קודיאק",
  "SCALA": "סקאלה",
  "ENYAQ": "אניאק",
  "IBIZA": "איביזה",
  "LEON": "לאון",
  "ARONA": "ארונה",
  "ATECA": "אטקה",
  "CIVIC": "סיוויק",
  "JAZZ": "גאז",
  "CRV": "CRV",
  "CR-V": "CRV",
  "HRV": "HRV",
  "HR-V": "HRV",
  "FIT": "גאז",
  "MAZDA2": "2",
  "MAZDA3": "3",
  "MAZDA 2": "2",
  "MAZDA 3": "3",
  "MAZDA 6": "6",
  "CX-3": "CX3",
  "CX-30": "CX30",
  "CX-5": "CX5",
  "MX-5": "MX5",
  "SWIFT": "סוויפט",
  "VITARA": "ויטארה",
  "JIMNY": "גימני",
  "IGNIS": "איגניס",
  "BALENO": "בלנו",
  "SX4": "SX4",
  "CLIO": "קליאו",
  "CAPTUR": "קפצור",
  "KOLEOS": "קולאוס",
  "MEGANE": "מגאן",
  "ARKANA": "ארקנה",
  "ZOE": "זואי",
  "208": "208",
  "2008": "2008",
  "3008": "3008",
  "308": "308",
  "5008": "5008",
  "C3": "C3",
  "C4": "C4",
  "BERLINGO": "ברלינגו",
  "CORSA": "קורסה",
  "ASTRA": "אסטרה",
  "CROSSLAND": "קרוסלנד",
  "MOKKA": "מוקה",
  "GRANDLAND": "גרנדלנד",
  "DUSTER": "דאסטר",
  "SANDERO": "סנדרו",
  "OUTLANDER": "אוטלנדר",
  "ASX": "ASX",
  "ECLIPSE CROSS": "אקליפס קרוס",
  "SPACE STAR": "ספייס סטאר",
  "COMPASS": "קומפאס",
  "RENEGADE": "רנגייד",
  "500": "500",
  "TIPO": "טיפו",
  "PANDA": "פנדה",
  "FOCUS": "פוקוס",
  "FIESTA": "פיאסטה",
  "PUMA": "פומה",
  "MUSTANG": "מוסטנג",
  "XV": "XV",
  "FORESTER": "פורסטר",
  "OUTBACK": "אווטבק",
  "XC40": "XC40",
  "XC60": "XC60",
  "XC90": "XC90",
  "V40": "V40",
  "V60": "V60",
  "S60": "S60",
  "S90": "S90",
  "EX30": "EX30",
  "A CLASS": "A CLASS",
  "C CLASS": "C CLASS",
  "E CLASS": "E CLASS",
  "S CLASS": "S CLASS",
  "GLA": "GLA",
  "GLC": "GLC",
  "GLE": "GLE",
  "A3": "A3",
  "A4": "A4",
  "A6": "A6",
  "Q3": "Q3",
  "Q5": "Q5",
  "E-TRON": "E TRON",
  "Q2": "Q2",
  "Q7": "Q7",
  "Q8": "Q8",
  "A1": "A1",
  "A5": "A5",
  "A7": "A7",
  "A8": "A8",
  "TT": "TT",
  "RS3": "RS3",
  "RS6": "RS6",
  "CLA": "CLA",
  "CLS": "CLS",
  "EQB": "EQB",
  "EQC": "EQC",
  "VITO": "ויטו",
  "SPRINTER": "ספרינטר",
  "1 SERIES": "סדרה 1",
  "2 SERIES": "סדרה 2",
  "3 SERIES": "סדרה 3",
  "4 SERIES": "סדרה 4",
  "5 SERIES": "סדרה 5",
  "X1": "X1",
  "X2": "X2",
  "X3": "X3",
  "X4": "X4",
  "X5": "X5",
  "X6": "X6",
  "I3": "I3",
  "I8": "I8",
  "CT200H": "CT200H",
  "ES300H": "ES300H",
  "IS": "IS300",
  "IS300": "IS300",
  "NX": "NX",
  "RX": "RX",
  "UX": "UX 200",
  "LBX": "LBX",
  "E-PACE": "E PACE",
  "F-PACE": "F PACE",
  "F-TYPE": "F TYPE",
  "EVOQUE": "איווק",
  "DISCOVERY SPORT": "דיסקברי ספורט",
  "VELAR": "וילאר",
  "911": "911",
  "CAYENNE": "קאיין",
  "MACAN": "מקאן",
  "PANAMERA": "פנמרה",
  "BOXSTER": "בוקסטר",
  "CAYMAN": "קאיימן",
  "TAYCAN": "טייקאן",
  "COUNTRYMAN": "קאנטרימן",
  "COOPER": "קופר S",
  "REXTON": "רקסטון",
  "ADAM": "אדם",
  "INSIGNIA": "אינסיגניה",
  "ZAFIRA": "זאפירה",
  "COMBO": "קומבו",
  "FORTWO": "FORFOUR",
  "FORFOUR": "FORFOUR",
  "L200": "L200",
  "PAJERO": "פאגרו",
  "500E": "500E",
  "500L": "500L",
  "500X": "500X",
  "DOBLO": "דובלו",
  "TIGGO 7": "טיגו 7",
  "TIGGO 8": "טיגו 8",
  "SRX": "SRX",
  "XT6": "XT6",
  "MACH-E": "מאך E",
  "MACH E": "מאך E",
  "MUSTANG MACH-E": "מאך E",
  "TRANSIT CUSTOM": "טרנזיט קאסטום",
  "AVENSIS": "אוונסיס",
  "VERSO": "ורסו",
  "PROACE CITY": "פרואייס סיטי",
  "STELVIO": "סטלביו",
  "GIULIA": "גוליה",
  "GIULIETTA": "גולייטה",
  "MITO": "מיטו",
  "TONALE": "רומיאו טונלה",
  "JUNIOR": "רומיאו גוניור",
  "T CROSS": "T CROSS טיקרוס",
  "TCROSS": "T CROSS טיקרוס",
  "AMAROK": "אמרוק",
  "SCIROCCO": "שירוקו",
  "JETTA": "גטה",
  "BEETLE": "חיפושית",
  "UP": "אפ",
  "CRAFTER": "קרפטר",
  "RAPID": "ראפיד",
  "SPACEBACK": "ספייסבק",
  "YETI": "יטי",
  "TAIGO": "טירוק",
  "T-ROC": "טירוק",
  "ALTEA": "אלטיאה",
  "TOLEDO": "טולדו",
  "FORMENTOR": "פורמנטור",
  "BORN": "קופרה בורן",
  "KANGOO": "קנגו",
  "FLUENCE": "פלואנס",
  "TWINGO": "טוינגו",
  "TRAFIC": "טראפיק",
  "D-MAX": "דימקס",
  "SIRION": "סיריון",
  "ALTO": "אלטו",
  "CELERIO": "סלריו",
  "SPLASH": "ספלאש",
  "S-CROSS": "אס קרוס",
  "GRAND VITARA": "גרנד ויטארה",
  "BRZ": "BRZ",
  "ZS": "ZS",
  "HS": "HS",
  "ATTO 3": "אטו 3",
  "GEOMETRY C": "גאומטרי C",
  "ORLANDO": "אורלנדו",
  "ACCORD": "אקורד",
  "INSIGHT": "אינסייט",
  "CRZ": "CRZ",
  "CR-Z": "CRZ",
  "FRV": "FRV",
  "GR YARIS": "GR יאריס",
  "GT86": "GT86",
  "GR86": "GT86",
  "86": "GT86",
};

export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);
    const plate = searchParams.get("plate");

    if (!plate) {
      return NextResponse.json(
        { error: "חסר מספר רכב" },
        { status: 400 }
      );
    }

    // Clean plate number — remove dashes, spaces, non-digits
    const cleanPlate = plate.replace(/[^0-9]/g, "");

    if (cleanPlate.length < 5 || cleanPlate.length > 8) {
      return NextResponse.json(
        { error: "מספר רכב לא תקין (5-8 ספרות)" },
        { status: 400 }
      );
    }

    // Query Israeli government API
    const url = `${GOV_API_URL}?resource_id=${RESOURCE_ID}&q=${cleanPlate}&limit=5`;
    const response = await fetch(url, {
      headers: { "Accept": "application/json" },
      next: { revalidate: 3600 }, // Cache for 1 hour
    });

    if (!response.ok) {
      console.error("Gov API error:", response.status);
      return NextResponse.json(
        { error: "שגיאה בשירות משרד התחבורה. נסה שוב מאוחר יותר." },
        { status: 502 }
      );
    }

    const data = await response.json();
    const records = data?.result?.records;

    if (!records || records.length === 0) {
      return NextResponse.json(
        { error: "לא נמצא רכב עם מספר זה" },
        { status: 404 }
      );
    }

    // Find exact match by plate number
    const record = records.find(
      (r: Record<string, string>) =>
        r.mispar_rechev?.toString().replace(/[^0-9]/g, "") === cleanPlate
    ) || records[0];

    const rawMake = (record.tozeret_nm || record.tozar || "").trim();
    const rawModel = (record.kinuy_mishari || record.degem_nm || "").trim();
    const year = parseInt(record.shnat_yitzur) || null;
    const color = (record.tzeva_rechev || "").trim();

    // Map manufacturer to Hebrew name used in our system
    const hebrewMake = normalizeMake(rawMake);

    // Map model to Hebrew name used in our system
    const modelUpper = rawModel.toUpperCase().trim();
    const hebrewModel = MODEL_MAP[modelUpper] || rawModel;

    return NextResponse.json({
      plate: cleanPlate,
      make: hebrewMake,
      makeOriginal: rawMake,
      model: hebrewModel,
      modelOriginal: rawModel,
      year,
      color,
    });
  } catch (error) {
    console.error("Vehicle lookup error:", error);
    return NextResponse.json(
      { error: "שגיאה בחיפוש. נסה שוב." },
      { status: 500 }
    );
  }
}
